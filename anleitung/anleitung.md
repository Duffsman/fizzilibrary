...
- Benutze kein `delay()`! Das führt dazu, dass das ganze Programm angehalten, und somit auch kein Ton mehr erzeugt wird. 
...
- Benutze keine *float* Zahlen, die braucht keiner und außerdem brauchen die viel länger in der Verarbeitung. Also auch kein `map()` benutzen, da das auch auf *floats* basiert. Als Ersatz gibt es eine Funktion die `intMap()` heißt, wie der Name schon andeutet funktioniert die mit integer Zahlen und ist somit nicht so ressourcenhungrig.
...
Das alles sind so genannte binäre Operatoren, die folgende Liste zeigt was diese genau machen. 
...
Beim Gestalten der eigenen Formel muss man das nicht wirklich verstehen, es kann aber hilfreich sein. An den binären Operatoren ist praktisch, dass mit ihnen auch etwas eingeschränkt "normale" mathematische Vorgänge ausgeführt werden können, die aber vom Microcontroller wesentlich schneller berechnet werden als bei der Verwendung der arithmetischen Operatoren. Hier die wichtigsten Beispiele:  
...
### die struktur eines fizzi sketch
...
Das Beispiel unten zeigt die Verwendung des sync Modes, sowie des Envelopes.
...
Zur Aktivierung des sync Eingang muss ganz am Anfang des Sketch der Befehl `#define USE_SYNC` eingefügt werden.
Außerdem kommen noch zwei Funktionen dazu die gefüllt werden müssen: `syncedOut()` und `syncPulse()`. Je nachdem, ob am Board der sync Mode aktiviert ist, werden die Samples in `defaultOut()` oder in `syncedOut()` berechnet. So kann man für die beiden Betriebsmodi unterschiedliche Verhalten implementieren. Die Funktion `syncPulse()` wird automatisch bei jedem neuen Synchronimpuls ausgeführt. Hier kommt also alles rein was passieren soll wenn es einen neuen Impuls gegeben hat. Um herauszufinden in welchem Betriebsmodus sich das Board befindet kann die boolsche Variable `isSynced` abgefragt werden.  
...
Die fizzi Library hält die Möglichkeit bereit einen Encoder zu verwenden, ohne sich um die Konfiguration der benötigten Interrupts kümmern zu müssen. Der Encoder muss an die Pins **P2** und **P3** angeschlossen werden. Am Anfang des Sketch teilt der Befehl `#define USE_ENCODER` dem Compiler mit, dass nun ein Encoder verwendet wird. Mit `encoder.getPosition()` lässt sich die aktuelle Postion auslesen, mit `encoder.setPosition()` kann der Wert des Encoders neu gesetzt werden.
