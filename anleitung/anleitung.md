...
- Versuche immer die kleinsten Datentypen zu verwenden, also wenn z.B. die Variable ohnehin nur von 0 bis 16 geht reicht es diese als *byte* zu deklarieren. So wird ohnehin knapper Speicherplatz gespart.
...
Beim gestalten der eingen Formel muss man das nicht wirklich verstehen, es kann aber hifreich sein. Das tolle an den binären Operatoren ist, dass man mit ihnen auch "normale" mathematische Vorgänge ausführen kann, nur geht das viel flinker. Hier die wichtigsten Beispiele:  
...
### ressources
Um sich mit den Formeln vertraut zu machen und um unkompliziert damit experimentieren zu können empfiehlt es sich einen Blick auf 
[http://wurstcaptures.untergrund.net/music/] zu werfen. Auf dieser Website kann man einfach Formeln eintragen, diese manipulieren und anhören. Die Samplerate muss wie beim fizzi Board auf *8000* eingestellt werden, trotzdem klingen die selben Formeln auf der Website anders als wenn man sie über fizzi anhört. Das liegt daran, dass das Onlinetool eine Btitiefe von 16 oder 24 Bit hat, bei fizzi sind es hingegen nur 8 Bit. Der folgende Abschnitt erläutert anhand von Beispielen die Grundlegenden Funktionen der *fizzi Library*, eine detailierte Referenz ist auch unter [https://duffsman.github.io/] zu finden.
...
Das folgende Beispiel zeigt wie wir den erzeugten Ton mit einem Potentiometer verändern können:

```c
#define USE_FAST_ADC // aktiviert schnelleren Analog/Digital Converter
#include<fizzi.h> // library in den Sketch einbinden 
...
In `updateControl()` wird der Wert vom Potentiometer ausgelsen und skaliert, die Formel wurde um einen Term erweitert, so dass *potVal* das Ergebnis beeinflusst.  
...
void setup() {
 ...
 en.setRetrigger(false); // Wenn der envelope gestartet wird, 
 						//kann er nicht erneut getriggert werden
...
Zur Einbindung von Buttons gibt es die `fizziButton` Klasse. Damit können angeschlossene Buttons entprellt werden und es werden einem eingie hilfreiche Funktionieren zum auslesen des Buttons zur verfügung gestellt.
...
void setup() {
  initfizzi(); // initialisierung von fizzi
  pinMode(BUTTON_PIN,INPUT_PULLUP);  // pinMode für Button Pin einstellen
  button.attach(BUTTON_PIN); // dem Button Objekt den zu lesenden Pin mitteilen
  }

void updateControl(){
	button.update();
	
	if(button.fell() == true) {	// diese Funktion wird immer 
							    //EIN mal ausgeführt wenn der Button 
	incr ++;				 	// gedrückt wurde
	if(incr > 16) incr = 1;
	}
		
	if(button.read() == LOW) { // diese Funktion wird so lange der Button gedrückt
	var = 4;				   // wird immer wieder ausgeführt
	} else var = 10;
